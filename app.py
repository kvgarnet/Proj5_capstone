...
def create_app(test_config=None):
    ...
    app = Flask(__name__)
    # based on proj1, load all app config from config.py
    app.config.from_object('config')
    # comment out below since secret key is moved in config.py, and accessed via .env in current directory
    # so we don't need to add the secret key in auth/.env file to make it clear
